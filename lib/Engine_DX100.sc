	...
	*new { arg context, doneCallback;
	...
	alloc {
		SynthDef(\dx100, {
			...
			v1 = 0, v2 = 0, v3 = 0, v4 = 0,
			rateScale = 0,
			drive = 0, hiss = 0, bits = 0, srate = 0, glitch = 0;
			...
			bits = Lag.kr(bits, 0.08).clip(0, 1);
			snd = SelectX.ar(bits, [
				snd,
				(snd * (2 ** (12 - (bits * 6)))).round(1.0)
					/ (2 ** (12 - (bits * 6)))
			]);
			srate = Lag.kr(srate, 0.08).clip(0, 1);
			snd = SelectX.ar(srate, [
				snd,
				Latch.ar(snd, Impulse.ar(srate.linexp(0.001, 1, 24000, 1500)))
			]);
